...
class WxValidate {
  ...
  __initDefaults() {
    this.defaults = {
      messages: {
        required: lang ? '这是必填字段。' : 'This field is required',
        email: lang ? '请输入有效的电子邮件地址。' : 'Email is invalid',
        tel: lang ? '请输入11位的手机号码。' : 'Phone Number is invalid',
        url: '请输入有效的网址。',
        date: '请输入有效的日期。',
        dateISO: '请输入有效的日期（ISO），例如：2009-06-23，1998/01/22。',
        number: lang ? '请输入有效的数字。' : 'Number reuired',
        digits: lang ? '只能输入数字。' : 'Number required',
        idcard: '请输入18位的有效身份证。',
        equalTo: this.formatTpl('输入值必须和 {0} 相同。'),
        contains: this.formatTpl('输入值必须包含 {0}。'),
        minlength: this.formatTpl(lang ? '最少要输入 {0} 个字符。' : '{0} Characters at least'),
        maxlength: this.formatTpl(lang ? '最多可以输入 {0} 个字符。' : '{0} Characters at most'),
